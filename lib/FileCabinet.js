...
module.exports.discoverConfigInfo = function (email, password, isSandbox) {
    ...
    var options = {
        ...
        headers: {
            'Authorization': 'NLAuth nlauth_email=' + encodeURIComponent(email) + ',nlauth_signature=' + encodeURIComponent(password)
        }
    };
...
module.exports.removeFile = function (filename, folder, callback) {
    init();
    search(filename, folder, function (err, resp, body) {
        xmlparser.parseString(body, function (err, result) {
            if (err) {
                throw err;
            }

            if (!result.Envelope.Body.searchResponse.searchResult.status.isSuccess) {
                console.log(result);
                return;
            }

            if (result.Envelope.Body.searchResponse.searchResult.totalRecords < 1) {
                console.log('No matching files found.');
                return;
            }

            var fileId = +result.Envelope.Body.searchResponse.searchResult.searchRowList.searchRow.basic.internalId.searchValue.internalId; // damn
            remove(fileId, function (err, resp, body) {
                xmlparser.parseString(body, function (err, result) {
                    return callback(err,result);
                });
            });
...
function remove(fileId, callback) {
    var nsRequest = makeSoapMsg("removeFileTemplate.xml", {
        fileId: fileId
    });

    // console.log(nsRequest);
    invokeWebService("delete", nsRequest, callback);
};

function search(filename, folder, callback) {
    var nsRequest = makeSoapMsg("searchFileTemplate.xml", {
        folder: folder || nsConf.folderid,
        filename: filename
    });
